...
class LLaVANuScenesDataset(NuScenesE2EDataset):
    ...
    def __init__(self,
        ...
        # Ego-dependency ablation: zero the ego-state + history fed to the prompt so the
        # model must plan from perception alone. Tests "is ego-status all you need?".
        if os.environ.get("ODV_ABLATE_EGO"):
            import numpy as _np
            n = 0
            for _tok, _v in self.cached_nuscenes_data.items():
                if 'gt_ego_lcf_feat' in _v:
                    _v['gt_ego_lcf_feat'] = _np.zeros_like(_np.asarray(_v['gt_ego_lcf_feat'], dtype=_np.float32)); n += 1
                if 'gt_ego_his_trajs' in _v:
                    _v['gt_ego_his_trajs'] = _np.zeros_like(_np.asarray(_v['gt_ego_his_trajs'], dtype=_np.float32))
            print(f">>> ODV_ABLATE_EGO: zeroed ego-state+history for {n} cached samples")
