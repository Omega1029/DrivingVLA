...
def planning_evaluation(pred_trajs_dict, subset=None, only_vehicle=True):
    ...
    for index, token in enumerate(tqdm(gt_trajs_dict.keys())):
        # Only evaluate tokens we actually produced predictions for. On full val
        # every GT token is predicted (no-op); on a subset (e.g. nuScenes-mini)
        # this restricts the metric to the predicted samples instead of KeyError-ing.
