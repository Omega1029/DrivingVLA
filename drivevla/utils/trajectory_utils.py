...
def retrieve_traj(text: str) -> List[Tuple[float, float]]:
    ...
    if len(coords_list) == 0:
        # Malformed/empty output (e.g. a heavily-quantized model emitting no parseable
        # coordinates). Return a degenerate "stay-in-place" trajectory rather than
        # crashing, so the metric still scores it (and the breakage shows up as L2).
        return [(0.0, 0.0)] * 6
