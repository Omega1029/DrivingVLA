...
variant is excluded pending a fix. (5) Fake-quant (dequantized) weights are used, so the
\emph{static} planner footprint is measured on-device (Jetson AGX Orin 64\,GB: $2.88$\,GB peak,
$14.9$ tok/s, group-wise INT4 packs the planner $0.72\to 0.19$\,GB) but a packed-INT4 \emph{speedup}
still requires a custom kernel and is not yet realized.
