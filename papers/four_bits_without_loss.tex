...
rotation restores full-precision accuracy and returns the degenerate count to base rate. The
practical conclusion is a division of labour: spend no effort on weight-grid engineering for
this driving policy at four bits, and spend it all on activation outliers. The methodological
conclusion is that closed-loop evaluation inherits the failure modes of the software it runs on,
and buys trustworthiness over open-loop metrics only if simulator state is controlled as carefully
as random seeds are.
\end{abstract}
\begin{IEEEkeywords}
...
\item A renderer-independent characterisation of what does break at 4 bits on this driving
policy---activation outliers---measured on a replay path structurally immune to the contamination
in Section~\ref{sec:mech} (Section~\ref{sec:activations}). Together with the corrected weight-axis
result, this gives a division of labour for 4-bit driving deployment: weight grids need no
engineering; activation outliers need a rotation.
\end{itemize}
...
accuracy and returns the degenerate-plan count to base rate. Activation outliers, not weight
granularity, are the binding constraint on this policy at four bits.
...
reveal. The activation results (Section~\ref{sec:activations}) are in-distribution replay and are 
not
themselves closed-loop; the title's second half is therefore a narrower claim than the
first---weight-axis freedom is a closed-loop, safety-graded result, while the activation-axis wall
is an accuracy result on a replay path. We scope this paper to the driving policy alone: an
earlier draft additionally claimed the activation axis transfers to a 7B manipulation policy on a
separate LIBERO harness, but given that a nominally identical statefulness bug produced a stable,
false effect in our own driving benchmark (Section~\ref{sec:mech}), we do not include that claim
here without first auditing the LIBERO harness for the same failure class. This policy is a single
checkpoint. Finally, closed-loop success throughout is measured with fake-quant weights, so we
report no measured latency win.
