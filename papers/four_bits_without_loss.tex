...
\title{\textbf{Four Bits Without Loss: INT4 Driving Policies Retain\\
Full-Precision Closed-Loop Safety Under Controlled Simulator State}}
...
We then identify what \emph{does} constrain 4-bit driving deployment. On a renderer-independent
replay path, quantizing activations to four bits with weights untouched inflates planning error
$47\times$ in-distribution, and a single data-free orthogonal rotation restores full-precision
accuracy; the same axis governs a 7B manipulation policy. Weights are free; activations are the
wall. The broader methodological point is that closed-loop evaluation inherits the failure modes
of the software it runs on, and buys trustworthiness over open-loop metrics only if simulator
state is controlled as carefully as random seeds are.
\end{abstract}
\begin{IEEEkeywords}
