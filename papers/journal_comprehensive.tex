...
scenarios). Fourth, we break the effect down by \textbf{scenario type} (frontal/side/stationary)
with cross-scenario variance, showing the collapse is uniform across types while group-wise recovery
is strongest on frontal scenarios and most variable on stationary ones. Fifth, we provide component
memory benchmarks and an edge-deployment analysis, including \emph{measured} on-device planner
latency and memory on a Jetson AGX Orin, showing the planner is trivial for the edge and perception
is the bottleneck.
\end{abstract}
...
distribute across scenario types? (Q4) What does this imply for edge deployment?
...
\subsection{Q3b: Scenario-type breakdown}
...
\midrule
frontal     & 5  & 73.9 & $\pm$5.4  & 77.0 & $\pm$5.5  & 3.3 \\
side        & 3  & 83.7 & $\pm$6.2  & 83.3 & $\pm$10.9 & 4.7 \\
stationary  & 8  & 74.4 & $\pm$6.5  & 67.7 & $\pm$20.3 & 3.5 \\
\midrule
overall     & 16 & 76.0 & $\pm$7.1  & 73.5 & $\pm$16.6 & 3.7 \\
\bottomrule
\end{tabular}
\end{table}

Frontal scenarios (approaching or crossing traffic) are where group-128 best recovers FP16:
mean $77.0\%$ vs $73.9\%$ FP16 (group slightly exceeds FP16, well within the $\pm5.5$
cross-scenario std---we do not claim group beats FP16). Stationary scenarios show the largest
absolute gap ($67.7\%$ vs $74.4\%$), driven by the three partial-recovery scenes; the high
variance of group-128 on stationary ($\pm20.3$) vs frontal ($\pm5.5$) reflects these outliers,
and illustrates why a single aggregate number is insufficient for safety evaluation. The
per-scenario table (Section~4.2) and the type breakdown together give a complete picture.
...
\paragraph{On-device measurement (Jetson AGX Orin 64\,GB).} We ran the planner on a real
NVIDIA Jetson AGX Orin Developer Kit (64\,GB, JetPack~6 / L4T~R36.4.7, CUDA capability~8.7,
PyTorch~2.8), loading the actual OpenDriveVLA-0.5B Qwen2.5 decoder weights (291/291 tensors) and
decoding $60$ trajectory tokens from a $1024$-token context (eager attention, default power mode,
median of $30$ runs). Because our quantizer is fake-quant (dequantized to FP16, no packed-INT4
GEMM in this build), latency and runtime memory are identical across precisions---confirming that
the deployable win is the \emph{static weight footprint}, not speed, until a packed kernel is added.
\begin{table}[h]\centering
...
\end{tabular}
\end{table}
The planner loads in $13.6$\,s and runs in under $3$\,GB on Orin---trivial for a 64\,GB board.
Group-wise INT4 shrinks the stored planner $3.9\times$ ($0.716\to 0.185$\,GB) for $\approx 0.1$
extra bits/weight over per-channel. Latency is flat across precisions (fake-quant); the
$\sim$15\,tok/s eager rate (clocks unpinned) is a conservative planner-only figure and excludes
perception. Realizing the memory \emph{and} a speedup on-device requires a packed-INT4 kernel
(e.g.\ Marlin/AWQ-style) plus a TensorRT path---left as deployment engineering. This measured
row supersedes the earlier device-class estimates and confirms the ``planner is free, perception
is the wall'' conclusion directly on the target hardware.
...
today. The evaluation recommendation---report generation coherence, per-scenario, alongside any
aggregate---follows from the fact that the aggregate collision metric is confounded and a single
mean hides the per-scenario spread documented in Table~\ref{tab:scentype}.
...
marked as such. The per-scenario coherence table is released as \texttt{benchmark14\_coherence.tsv},
and the on-device Orin measurements as \texttt{orin/results/}, so every table here can be
...
frontal, side, and stationary scenarios. The recommended evaluation metric for quantized driving
planners is per-scenario generation coherence, not open-loop L2 and not the confounded collision
count. Future work: more scenes and a stronger renderer to test generality; a direct
malformed-output-rate protocol; per-frame trajectory-quality and degenerate-state-recovery metrics,
which our current logs do not support; a corrected activation-aware quantizer; and on-Orin
throughput with a TensorRT-INT8 perception port.
