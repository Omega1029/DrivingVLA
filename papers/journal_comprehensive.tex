...
empirical result, replicated across \emph{two} real-camera NeuroNCAP scenes: under the
deployment-domain shift of neural rendering, \emph{naive per-output-channel} INT4 collapses
trajectory-text generation ($0.0\%$ and $8.6\%$ well-formed frames, planner frozen), whereas
\emph{group-wise} INT4 (group 128, $\approx 0.1$ extra bits/weight) recovers the bulk of FP16-level
generation coherence ($72.9\%$ vs $74.3\%$; $57.9\%$ vs $72.4\%$). The collapse
is a quantization\,$\times$\,domain-shift interaction governed by scale granularity, not bit-width,
since the same per-channel INT4 is lossless in-distribution. Fourth, we provide component memory
benchmarks and an edge-deployment analysis (Jetson Orin, Raspberry Pi) showing the planner is
trivial for the edge and perception is the bottleneck. We are explicit throughout about scope: the
closed-loop results are two-scene, group-wise recovery is partial on the harder scene, and one scene
(0796) is failed by \emph{every} configuration including FP16---so we lead with the coherence metric
...
\begin{table}[h]\centering
\caption{NeuroNCAP closed-loop with \emph{real-camera} NeuRAD rendering (tcnn), 10 paired seeds per
scene. Generation coherence = well-formed (non-degenerate) planned-frame rate; NCAP score and
collisions are secondary, confounded proxies.}
\begin{tabular}{lcccccc}
\toprule
 & \multicolumn{3}{c}{scene-0103 (frontal)} & \multicolumn{3}{c}{scene-0796 (stationary)} \\
\cmidrule(lr){2-4}\cmidrule(lr){5-7}
Config & coherence & NCAP & coll. & coherence & NCAP & coll. \\
\midrule
...
\end{tabular}
\end{table}
Across both scenes, per-channel INT4 collapses generation---degenerate (all-zero/malformed) plans on
essentially every frame ($0.0\%$ and $8.6\%$ well-formed)---while group-wise INT4 recovers the bulk
of FP16's coherence ($72.9\%$ vs $74.3\%$ on 0103; $57.9\%$ vs $72.4\%$ on 0796). The same
per-channel INT4 is lossless in-distribution (Table 1), so the failure is a
quantization\,$\times$\,domain-shift interaction. We are careful not to over-claim equivalence:
group-wise \emph{matches} FP16 on 0103 but only \emph{partially} recovers on 0796.
...
\textbf{Two scenes.} Closed-loop results are on NeuroNCAP scene-0103 (frontal) and scene-0796
(stationary); we establish the mechanism and replicate it once, not fleet generality. The remaining
NeuRAD-reconstructed scenes require gated nuScenes trainval metadata. \textbf{Renderer domain gap.}
The rendered inputs are out-of-distribution relative to real nuScenes; even FP16 fails scene-0796's
collision test ($10/10$), so absolute NCAP/collision numbers are not comparable to in-distribution
operation and our claims are explicitly relative (group $\approx$ FP16 $\gg$ per-channel on
coherence). \textbf{Partial recovery.} Group-wise INT4 matches FP16 coherence on 0103 but only
recovers it partially on 0796 ($57.9\%$ vs $72.4\%$); we do not claim exact equivalence.
