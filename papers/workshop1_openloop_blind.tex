...
\title{\textbf{Open-Loop Metrics Are Blind: Why Quantized Autonomous-Driving\\
Vision--Language--Action Policies Must Be Certified Closed-Loop}}
...
\paragraph{On the deployment target.} We replayed the captured planner inputs on a real Jetson AGX
Orin 64\,GB (JetPack~6, torch~2.8) and re-scored open-loop L2 (40-sample mini subset, ST-P3). The
Orin reproduces the A100 to $\approx 1$\,cm mean (FP16), and INT4 remains ``lossless'' on-device
(FP16 $0.87$\,m vs group-INT4 $0.90$\,m at 3\,s)---even though the per-sample W4 plans already differ
from FP16 by $14$--$24$\,cm. The blindness travels to the edge hardware: it is the metric, not the
machine.
