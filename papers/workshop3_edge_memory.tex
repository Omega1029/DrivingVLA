...
% Footprints measured from the live model node (load + inference); deployment
% feasibility reasoned from device specs. Marked clearly where estimated.
% ============================================================================
...
present in-memory benchmarks for the components and quantization levels, and argue that edge VLA
...
\begin{table}[h]\centering
\caption{Deployment feasibility. Memory ``fits'' is from the $3$--$5$\,GB footprint; throughput is
reasoned from device class (estimates, marked $^\dagger$).}
\begin{tabular}{lccc}
\toprule
Target & memory & planner & perception (real-time) \\
\midrule
A100 (reference)      & yes & trivial & yes \\
Jetson AGX Orin 32/64\,GB & yes & trivial & no FP32; needs TRT-INT8 port$^\dagger$ \\
