...
# Env override allows a nuScenes-mini bring-up without editing the config.
# (The NuScenes *version* is read from the infos pkl's metadata, not from here,
#  so the mini subset infos is stamped with version 'v1.0-mini'.)
#   ODV_VAL_INFO=nuscenes_infos_temporal_val_mini.pkl
import os as _os
ann_file_train=info_root + f"nuscenes_infos_temporal_train.pkl"
ann_file_val=info_root + _os.environ.get("ODV_VAL_INFO", "nuscenes_infos_temporal_val.pkl")
ann_file_test=info_root + _os.environ.get("ODV_VAL_INFO", "nuscenes_infos_temporal_val.pkl")
