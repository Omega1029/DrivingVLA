...
class PansegformerHead(SegDETRHead):
    ...
    def forward_test(self,
        ...
        if gt_lane_masks is None:
            # Closed-loop (rendered) inference has no GT map; these IoU metrics are eval-only and
            # are popped before the VLM tokens are built, so skip them when GT is unavailable.
            ret_iou = {}
        else:
          with torch.no_grad():
    ...
    def get_bboxes(
        ...
        for img_id in range(len(img_metas)):
            ...
            output_query_things = query_inter_things[-1].squeeze(0)  # query from the last decoder layer
            output_query_stuff = query_inter_stuff[-1].squeeze(0)  # query from the last decoder layer
